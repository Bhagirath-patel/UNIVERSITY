<html>
	...
	<body>
	  <div class="container-fluid" >
		  ...
		  <div class="row" style="height:20px"></div>
		   ...
		   <div class="col-sm-4 w3-center w3-border-bottom w3-border-pink "><b  style="font-Size:25px;">Result</b ></div>
		   <div class="col-sm-4"></div>
		</div>
	...
	<div class="row">
	  ...
	  <div class="col-sm-10">
			<!--    <?php
						if(isset($_GET["wrong"])){
							echo '<b class="w3-text-pink">'."invalid Roll No".'</b>';
						}
					?> 
			-->
					<form method="post" action="getresult.php">
					<div class="row">
					<label class="col-sm-3">Select Result Type</label>
					<div class="col-sm-6">
								<select name="result" class="form-control">
										<option>Select Result Type:-</option>
					<?php
						include("database.php");
						$rs=mysql_query("select * from resultbtech");
						while($r=mysql_fetch_array($rs)){
							?>
								<option value="<?=$r[1]?> <?=$r[5]?> <?=$r[4]?>"><?=$r[1]?> <?=$r[5]?> year <?=$r[4]?> semester</option>
								
							<?php
						}
						include("database.php");
						$rs=mysql_query("select * from resultbsc");
						while($r=mysql_fetch_array($rs)){
							?>
								<option value="<?=$r[1]?> <?=$r[3]?>"><?=$r[1]?> <?=$r[3]?> year</option>
								
							<?php
						}
						include("database.php");
						$rs=mysql_query("select * from resultmsc");
						while($r=mysql_fetch_array($rs)){
							?>
								<option value="<?=$r[1]?> <?=$r[3]?>"><?=$r[1]?> <?=$r[3]?> year</option>
